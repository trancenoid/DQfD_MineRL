...
class DQfD:
    def __init__(self, env, config, demo_transitions=None):
        ...
        self.action_batch = tf.compat.v1.placeholder("int32", [None])
        self.y_input = tf.compat.v1.placeholder("float", [None, self.action_dim])
        self.ISWeights = tf.compat.v1.placeholder("float", [None, 1])
        self.n_step_y_input = tf.compat.v1.placeholder("float", [None, self.action_dim])  # for n-step reward
        self.isdemo = tf.compat.v1.placeholder("float", [None])
        self.eval_input = tf.compat.v1.placeholder("float", [None, self.state_dim])
        self.select_input = tf.compat.v1.placeholder("float", [None, self.state_dim])
    ...
    def add_demo_to_memory(self, demo_transitions):
        ...
        print("adding demo data to memory")
        for t in tqdm(demo_transitions):
    ...
    def build_layers(self, state, c_names, units_1, units_2, w_i, b_i, reg=None):
        ...
        with tf.compat.v1.variable_scope('l1'):
            w1 = tf.compat.v1.get_variable('w1', [self.state_dim, units_1], initializer=w_i, collections=c_names, regularizer=reg)
            b1 = tf.compat.v1.get_variable('b1', [1, units_1], initializer=b_i, collections=c_names, regularizer=reg)
            dense1 = tf.nn.relu(tf.matmul(state, w1) + b1)
        with tf.compat.v1.variable_scope('l2'):
            w2 = tf.compat.v1.get_variable('w2', [units_1, units_2], initializer=w_i, collections=c_names, regularizer=reg)
            b2 = tf.compat.v1.get_variable('b2', [1, units_2], initializer=b_i, collections=c_names, regularizer=reg)
            dense2 = tf.nn.relu(tf.matmul(dense1, w2) + b2)
        with tf.compat.v1.variable_scope('l3'):
            w3 = tf.compat.v1.get_variable('w3', [units_2, a_d], initializer=w_i, collections=c_names, regularizer=reg)
            b3 = tf.compat.v1.get_variable('b3', [1, a_d], initializer=b_i, collections=c_names, regularizer=reg)
            dense3 = tf.matmul(dense2, w3) + b3
        return dense3

    @lazy_property
    def Q_select(self):
        with tf.compat.v1.variable_scope('select_net') as scope:
            c_names = ['select_net_params', tf.compat.v1.GraphKeys.GLOBAL_VARIABLES]
            w_i = tf.compat.v1.random_uniform_initializer(-0.1, 0.1)
            b_i = tf.compat.v1.constant_initializer(0.1)
            reg = tf.keras.regularizers.l2(l=0.5 * (0.2))  # Note: only parameters in select-net need L2
            return self.build_layers(self.select_input, c_names, 24, 24, w_i, b_i, reg)

    @lazy_property
    def Q_eval(self):
        with tf.compat.v1.variable_scope('eval_net') as scope:
    ...
    @lazy_property
    def loss(self):
        l_dq = tf.reduce_mean(input_tensor=tf.math.squared_difference(self.Q_select, self.y_input))
        l_n_dq = tf.reduce_mean(input_tensor=tf.math.squared_difference(self.Q_select, self.n_step_y_input))
        # l_n_step_dq = self.loss_n_step_dq(self.Q_select, self.n_step_y_input)
        l_jeq = self.loss_jeq(self.Q_select)
        l_l2 = tf.reduce_sum(input_tensor=[tf.reduce_mean(input_tensor=reg_l) for reg_l in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES)])
        return self.ISWeights * tf.reduce_sum(input_tensor=[l * lambd for l, lambd in zip([l_dq, l_n_dq, l_jeq, l_l2], self.config.LAMBDA)])

    @lazy_property
    def abs_errors(self):
        return tf.reduce_sum(input_tensor=tf.abs(self.y_input - self.Q_select), axis=1)  # only use 1-step R to compute abs_errors
    ...
    def update_target_net(self):
        select_params = tf.compat.v1.get_collection('select_net_params')
        eval_params = tf.compat.v1.get_collection('eval_net_params')
        return [tf.compat.v1.assign(e, s) for e, s in zip(eval_params, select_params)]
    ...
    def perceive(self, transition):
        self.replay_memory.store(np.array(transition,dtype=object))
    ...
    def train_Q_network(self, pre_train=False, update=True):
        ...
        for i in range(self.config.BATCH_SIZE):
            ...
            # change to softmax output to fix error
            temp_0 = np.copy(temp)
            # add 1-step reward
